...
N_OBS_PER_SUBJ  <- 20
SIGMA_B         <- 0.5
SUBJ_SIZES      <- c(10, 20, 30, 50, 80, 120)
MASTER_SEED     <- 42
OUT_FILE        <- "results/sample_size_sweep.csv"
N_CORES         <- parallel::detectCores() - 1

dir.create("results", showWarnings = FALSE, recursive = TRUE)

# ── Generate data fresh per n_subj (need different-sized pools) ────────────────
generate_data <- function(n_subj, base_seed) {
  set.seed(base_seed)
...
# ── Single seed worker ─────────────────────────────────────────────────────────
run_one_seed <- function(seed, n_subj, dat) {
  set.seed(seed)
  n_train <- round(n_subj * 0.8)
  ...
  if (is.null(fit)) {
    return(data.frame(seed = seed, n_subj = n_subj,
  ...
  data.frame(seed = seed, n_subj = n_subj,
...
# ── Run ───────────────────────────────────────────────────────────────────────
cat("Running sample-size sweep: p =", P, ",", length(SUBJ_SIZES), "sizes ×", N_SEEDS, "seeds\n")

cl <- makeCluster(N_CORES)
clusterEvalQ(cl, { library(lme4); library(performance) })

results <- list()
for (ns_val in SUBJ_SIZES) {
  cat("  n_subj =", ns_val, "...")
  pool <- generate_data(ns_val, MASTER_SEED + ns_val)
  clusterExport(cl, c("pool", "P", "N_OBS_PER_SUBJ", "run_one_seed", "ns_val"), envir = environment())
